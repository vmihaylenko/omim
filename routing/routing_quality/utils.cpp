...
public:
  ...
  routing_quality::RoutePoints operator()(routing_quality::RouteParams && params)
  {
    using namespace routing;

    unique_ptr<storage::CountryInfoGetter> cig = storage::CountryInfoReader::CreateCountryInfoReader(GetPlatform());
    CHECK(cig, ());
    ...
    IndexRouter indexRouter(params.m_type, false /* load altitudes*/, *countryParentGetter, countryFileGetter,
                            getMwmRectByName, m_numMwmIds, MakeNumMwmTree(*m_numMwmIds, infoGetter),
                            cache, m_dataSource);

    RouterDelegate delegate;
    Route route("" /* router */, 0 /* routeId */);

    routing_quality::RoutePoints points = routing_quality::FromLatLon(params.m_coords);
    auto const result = indexRouter.CalculateRoute(Checkpoints(move(points)), m2::PointD::Zero(),
                                                   false /* adjustToPrevRoute */, delegate, route);

    CHECK_EQUAL(result, RouterResultCode::NoError, ());
    CHECK(route.IsValid(), ());

    return route.GetPoly().GetPoints();
  }
...
{
  CHECK_GREATER_OR_EQUAL(params.m_coords.size(), 2, ());
  PointsGetter get;
  return get(move(params));
}

RoutePoints FromLatLon(vector<ms::LatLon> const & coords)
{
  CHECK(!coords.empty(), ());
  RoutePoints ret;
