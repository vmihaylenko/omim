...
double CalcTrafficFactor(SpeedGroup speedGroup, Purpose)
{
...
double GetPedestrianClimbPenalty(double tangent, Purpose)
{
  if (tangent < 0)
...
double GetBicycleClimbPenalty(double tangent, Purpose)
{
  if (tangent <= 0)
    return 1.0;
...
double GetCarClimbPenalty(double /* tangent */, Purpose) { return 1.0; }
                              ...
                              GetClimbPenalty && getClimbPenalty)
{
  ...
  VehicleModelInterface::SpeedKMpH const & speed = road.GetSpeed();
  ...
  return timeSec * getClimbPenalty(altitudeDiff / distance, purpose);
}
}  // namespace
...
public:
  ...
  double CalcSegmentTransitTime(Segment const & segment, RoadGeometry const & road) const override
  {
...
double CarEstimator::CalcSegmentTransitTime(Segment const & segment, RoadGeometry const & road) const
{
...
double CarEstimator::CalcSegment(Purpose purpose, Segment const & segment, RoadGeometry const & road) const
{
  ...
  if (m_trafficStash)
  {
    ...
    double const trafficFactor = CalcTrafficFactor(speedGroup, purpose);
...
shared_ptr<EdgeEstimator> EdgeEstimator::Create(VehicleType vehicleType,
                                                ...
                                                shared_ptr<TrafficStash> trafficStash)
{
  return Create(vehicleType, vehicleModel.GetMaxSpeed().m_weight, vehicleModel.GetOffroadSpeed(),
                trafficStash);
}
}  // namespace routing
